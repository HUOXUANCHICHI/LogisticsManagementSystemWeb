<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8"/>
    <meta http-equiv="X-UA-Compatible" content="IE=edge"/>
    <meta
            name="viewport"
            content="width=device-width, initial-scale=1, shrink-to-fit=no"
    />
    <meta name="description" content="A project modified by a SIT student."/>
    <meta name="author" content="${username}"/>

    <title>物流信息管理系统-用户信息修改</title>

    <!-- Custom fonts for this template-->
    <link
            href="vendor/fontawesome-free/css/all.min.css"
            rel="stylesheet"
            type="text/css"
    />
    <!-- <link
      href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
      rel="stylesheet"
    /> -->

    <!-- Custom styles for this template-->
    <link href="css/sb-admin-2.min.css" rel="stylesheet"/>
</head>

<body id="page-top">
<!-- Page Wrapper -->
<div id="wrapper">


    <!-- Content Wrapper -->
    <div id="content-wrapper" class="d-flex flex-column">
        <!-- Main Content -->
        <div id="content">

            <!-- Begin Page Content -->
            <div class="container-fluid">
                <!-- Page Heading -->
                <h1 class="h3 mb-4 text-gray-800">用户信息修改</h1>
            </div>
            <!-- Custom -->
            <div class="justify-content-center">
                <form name="form" action="UsersUpdateServlet" method="post">
                    <div class="user">
                        <div class="form-group">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                <input
                                        type="text"
                                        class="form-control form-control-user"
                                        id="userId"
                                        aria-describedby="emailHelp"
                                        placeholder="用户编号"
                                        value=""
                                        name="userId"
                                        readonly
                                />
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                <input
                                        type="text"
                                        class="form-control form-control-user"
                                        id="username"
                                        aria-describedby="emailHelp"
                                        placeholder="用户名"
                                        name="username"
                                        value="${users.username}"
                                />
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                <input
                                        type="text"
                                        class="form-control form-control-user"
                                        id="userPwd"
                                        aria-describedby="emailHelp"
                                        placeholder="用户密码"
                                        name="userPwd"
                                        value="${users.userPwd}"
                                />
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                <input
                                        type="text"
                                        class="form-control form-control-user"
                                        id="userContact"
                                        aria-describedby="emailHelp"
                                        placeholder="用户联系方式"
                                        name="userContact"
                                        value="${users.userContact}"
                                />
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-12 mb-3 mb-sm-0">
                                <div class="form-check form-check-inline">
                                    <c:choose>
                                        <c:when test="${users.accountRole==0}">
                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio1" value="0" checked>
                                            <label class="form-check-label" for="inlineRadio1">用户</label>

                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio2" value="1">
                                            <label class="form-check-label" for="inlineRadio2">管理员</label>

                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio3" value="2">
                                            <label class="form-check-label" for="inlineRadio3">员工</label>
                                        </c:when>
                                        <c:when test="${users.accountRole==1}">
                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio1" value="0">
                                            <label class="form-check-label" for="inlineRadio1">用户</label>

                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio2" value="1" checked>
                                            <label class="form-check-label" for="inlineRadio2">管理员</label>

                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio3" value="2">
                                            <label class="form-check-label" for="inlineRadio3">员工</label>
                                        </c:when>
                                        <c:when test="${users.accountRole==2}">
                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio1" value="0">
                                            <label class="form-check-label" for="inlineRadio1">用户</label>

                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio2" value="1">
                                            <label class="form-check-label" for="inlineRadio2">管理员</label>

                                            <input class="form-check-input" type="radio" name="accountRole"
                                                   id="inlineRadio3" value="2" checked>
                                            <label class="form-check-label" for="inlineRadio3">员工</label>
                                        </c:when>
                                    </c:choose>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-3 mb-3 mb-sm-0">
                            <button class="btn btn-primary btn-user btn-block">
                                更新
                            </button>
                        </div>
                    </div>
                </form>
            </div>
            <!-- End of Custom -->
            <!-- /.container-fluid -->
        </div>
        <!-- End of Main Content -->

        <!-- Footer -->

        <!-- End of Footer -->
    </div>
    <!-- End of Content Wrapper -->
</div>
<!-- End of Page Wrapper -->

<!-- Scroll to Top Button-->
<a class="scroll-to-top rounded" href="#page-top">
    <i class="fas fa-angle-up"></i>
</a>

<!-- Logout Modal-->

<!-- Bootstrap core JavaScript-->
<script src="vendor/jquery/jquery.min.js"></script>
<script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

<!-- Core plugin JavaScript-->
<script src="vendor/jquery-easing/jquery.easing.min.js"></script>

<!-- Custom scripts for all pages-->
<script src="js/sb-admin-2.min.js"></script>
<script src="js/getUrlVariable.js"></script>
<script>
    document.getElementById("userId").value = getUrlVariable("userId");
</script>
</body>
</html>
